    ...
    if (existingPerson) {
      ...
      if (confirmUpdate) {
        ...
        PersonService.update(existingPerson.id, updatedPerson)
          ...
          .catch((error) => {
            handleInfoMessageErrorUpdate(
              `Info of ${updatedPerson.name} has been already deleted from the list.`,
              error
            );
            setPersons(
      ...
      handleInfoMessageErrorUpdate(
        `Person validation failed: Name "${newName}" is shorter than the minimum allowed length for the name (3).`
      );
      setNewName("");
      setNewNumber("");
    } else if (!/^\d{3}-\d{7}$/.test(newNumber)) {
      handleInfoMessageErrorUpdate(
        "Phone number should be in the format XXX-XXXXXXX"
      );
    } else {
      const newPerson = {
        name: newName,
      ...
      PersonService.create(newPerson)
        .then((createdPerson) => {
          ...
          handleMessageToGo(
            `Person ${newPerson.name} has been added to the list`
          );
          setNewName("");
          setNewNumber("");
        })
